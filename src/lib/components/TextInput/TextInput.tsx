...
export interface ReuseUITextInputTheme {
  base: string;
  addon: string;
  field: {
    base: string;
...
export interface TextInputSizes extends Pick<ReuseUISizes, 'sm' | 'md' | 'lg'> {
  [key: string]: string;
}
...
export interface TextInputProps extends Omit<ComponentProps<'input'>, 'ref' | 'color'> {
  ...
  theme?: DeepPartial<ReuseUITextInputTheme>;
}
