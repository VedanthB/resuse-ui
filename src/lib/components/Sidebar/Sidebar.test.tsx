...
import type { ReuseUITheme } from '../ReuseUI';
...
const collapseIcons = () => screen.getAllByTestId('ReuseUI-sidebar-collapse-icon');

const collapseLabels = () => screen.getAllByTestId('ReuseUI-sidebar-collapse-label');

const cta = () => screen.getByText('Some content');

const itemContents = () => screen.getAllByTestId('ReuseUI-sidebar-item-content');

const itemGroups = () => screen.getAllByTestId('ReuseUI-sidebar-item-group');

const icons = () => screen.getAllByTestId('ReuseUI-sidebar-item-icon');

const items = () => screen.getAllByRole('listitem');

const itemsContainers = () => screen.getAllByTestId('ReuseUI-sidebar-items');

const logo = () => screen.getByLabelText('ReuseUI');
...
describe('Components / Sidebar', () => {
  describe('A11y', () => {
    ...
    it('should use text content as accessible name in `Sidebar.Logo`', () => {
      ...
      expect(logo()).toHaveAccessibleName('ReuseUI');
    });

    it('should use `imgAlt` as alternative text for image in `Sidebar.Logo` given `img=".." and imgAlt=".."`', () => {
      render(<TestSidebar />);

      const logoImg = screen.getByAltText('ReuseUI logo');

      expect(logoImg).toHaveAccessibleName('ReuseUI logo');
    });
  });
});
...
describe('Theme', () => {
  it('should use custom classes', () => {
    const theme: DeepPartial<ReuseUITheme> = {
      sidebar: {
        root: {
          base: 'bg-gray-100',
          collapsed: {
